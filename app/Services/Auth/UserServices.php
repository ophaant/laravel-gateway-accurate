<?php

namespace App\Services\Auth;

use App\Exceptions\handleDatabaseException;
use App\Helpers\errorCodes;
use App\Interfaces\Auth\AuthInterfaces;
use App\Traits\ApiResponse;
use Exception;
use Illuminate\Support\Facades\Log;

class UserServices
{
    use ApiResponse;

    protected $authInterfaces;

    public function __construct(AuthInterfaces $authInterfaces)
    {
        $this->authInterfaces = $authInterfaces;
    }

    public function getAll()
    {
        try {
            $data = $this->authInterfaces->getAll();

            if (!is_object($data)) {
                return $data;
            }
            return $this->successResponse($data, 200, 'User List Successfully');
        }catch (\PDOException $e) {
            Log::debug($e->getMessage());
            throw new handleDatabaseException($e->errorInfo, $e->getMessage());
        } catch (Exception $e) {
            Log::debug($e->getMessage());
            return $this->errorResponse($e->getMessage(), 500, errorCodes::CODE_WRONG_ERROR);
        }
    }

    public function getById($id)
    {
        try {
            $data = $this->authInterfaces->getById($id);

            if (!is_object($data)) {
                return $data;
            }
            return $this->successResponse($data, 200, 'User Details Successfully');
        }catch (\PDOException $e) {
            Log::debug($e->getMessage());
            throw new handleDatabaseException($e->errorInfo, $e->getMessage());
        } catch (Exception $e) {
            Log::debug($e->getMessage());
            return $this->errorResponse($e->getMessage(), 500, errorCodes::CODE_WRONG_ERROR);
        }
    }

    public function update($id, array $data)
    {
        try {
            $data = $this->authInterfaces->update($id, $data);

            if (!is_object($data)) {
                return $data;
            }
            return $this->successResponse($data, 200, 'User Updated Successfully');
        }catch (\PDOException $e) {
            Log::debug($e->getMessage());
            throw new handleDatabaseException($e->errorInfo, $e->getMessage());
        } catch (Exception $e) {
            Log::debug($e->getMessage());
            return $this->errorResponse($e->getMessage(), 500, errorCodes::CODE_WRONG_ERROR);
        }
    }

    public function delete($id)
    {
        try {
            $data = $this->authInterfaces->delete($id);

            if ($data->getStatusCode() != 200) {
                return $data;
            }
            return $this->successResponse(null, 200, 'User Deleted Successfully');
        }catch (\PDOException $e) {
            Log::debug($e->getMessage());
            throw new handleDatabaseException($e->errorInfo, $e->getMessage());
        } catch (Exception $e) {
            Log::debug($e->getMessage());
            return $this->errorResponse($e->getMessage(), 500, errorCodes::CODE_WRONG_ERROR);
        }
    }
}
